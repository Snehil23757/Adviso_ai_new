...
def render_email_template(template: str, context: dict[str, Any]) -> RenderedEmail:
    ...
    plan = escape(str(context.get("plan_name") or "Free Trial"))
    ...
    if template == "password_reset":
        body = f"""
          <h1 style="margin:0;text-align:center;font-size:34px;line-height:40px;letter-spacing:-0.04em;color:#071536;">Reset your Adviso AI password</h1>
          <p style="margin:16px auto 0;max-width:500px;text-align:center;font-size:16px;line-height:26px;color:{MUTED};">Use the secure link below to choose a new password. The link is generated by Firebase Authentication and expires automatically.</p>
          {_button("Reset Password", reset_url)}
          <p style="margin:22px 0 0;text-align:center;font-size:13px;line-height:20px;color:{MUTED};">If you did not request this, you can ignore this email.</p>
        """
        return RenderedEmail(
            subject="Reset your Adviso AI password",
            html=_base_layout("Reset your password", "Reset your Adviso AI password.", body),
            text=f"Reset your Adviso AI password: {reset_url}",
    ...
    if template == "trial_started":
        ...
        """
        return RenderedEmail(
            subject="Your Adviso AI trial is active",
            html=_base_layout("Trial started", "Your Adviso AI trial is ready.", body),
            text=f"Your Adviso AI trial is active. Launch workspace: {launch_url}",
            from_address=get_settings().email_from_welcome,
        )
    ...
    if template == "registration_success":
        return RenderedEmail(
            ...
            text=f"Welcome to Adviso AI. Your intelligent business workspace is ready: {launch_url}",
            from_address=get_settings().email_from_welcome,
        )
